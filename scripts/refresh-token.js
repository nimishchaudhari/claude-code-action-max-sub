...
function updateGitHubSecret(secretName, secretValue, owner, repo) {
  try {
    ...
    execSync(`gh secret set ${secretName} --body "${secretValue}" --repo ${owner}/${repo}`, {
      stdio: 'inherit'
    });
...
async function main() {
  try {
    ...
    if (process.env.UPDATE_GITHUB_SECRET === 'true') {
      ...
      // Update access token
      updateGitHubSecret('CLAUDE_ACCESS_TOKEN', tokenResponse.access_token, owner, repo);
      
      // Update refresh token if a new one was provided
      if (tokenResponse.refresh_token) {
        updateGitHubSecret('CLAUDE_REFRESH_TOKEN', tokenResponse.refresh_token, owner, repo);
      }
    }

    // Output the new token for the current workflow
    console.log(`::set-output name=access_token::${tokenResponse.access_token}`);
    
    // Also set it as an environment variable for subsequent steps
    console.log(`::add-mask::${tokenResponse.access_token}`);
    console.log(`CLAUDE_ACCESS_TOKEN=${tokenResponse.access_token} >> $GITHUB_ENV`);
